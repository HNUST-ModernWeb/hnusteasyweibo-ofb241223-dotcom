...
<h1 align="center">HNUST Easy WeiBo</h1>
...
`HNUST Easy WeiBo` 是一个围绕校园场景设计的社交分享平台课程项目，目标不是只做一个“能展示页面的前端原型”，而是完成一套真正可运行的全栈系统：

- 前端基于 `Vue 3 + Vite + TypeScript`
- 后端基于 `Spring Boot 3 + MyBatis + MySQL 8`
- 已打通用户认证、内容发布、评论互动、通知、私聊、后台治理和 AI 对话

它既适合用作课程答辩展示，也具备较强的 GitHub 项目展示价值。
...
| 课程层级 | README 原要求 | 当前实现 |
| --- | --- | --- |
| 基础级 | 文字分享、浏览功能 | 已完成：首页信息流、发帖、详情、浏览、个人主页 |
| 标准级 | 图片上传、评论、用户认证 | 已完成：图片上传、评论系统、登录注册找回密码 |
| 挑战级 | 点赞、通知、权限控制、特色功能 | 已完成：点赞、收藏、转发、通知、话题、表情、管理员控制台、AI 助手 |
| 扩展能力 | API 文档、统一异常处理、安全性、前后端整合 | 已完成：Swagger UI、全局异常处理、JWT、BCrypt、CORS、真实前后端联调 |

这个项目当前的能力已经明显超过原始课程 README 中的“挑战级”要求。
...
cd hnusteasyweibo-ofb241223-dotcom
```
...
`HNUST Easy WeiBo` 的最终形态不是“只有前端页面的社交原型”，而是一套已经完成主要业务闭环的课程级全栈系统：
...
如果从课程项目视角看，它已经足够完整；如果从工程化产品视角看，它仍有继续演进的空间，例如：
...
但作为一份课程作品，它已经具备了很强的展示性、完整性和可演示性。
